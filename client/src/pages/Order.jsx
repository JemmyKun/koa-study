...
class Order extends Component {
  constructor(props) {
    ...
    this.columns = [
      ...
      },
      {
        ...
        render: updateTime => (
          <span>{format(updateTime, "YYYY-MM-DD HH:mm:ss")}</span>
        )
      },
      {
        title: "订单状态",
        ...
        render: (status, item) => {
          ...
          return <span className={"handle-list-name"}>{showIcon}</span>;
        }
      },
      {
        title: "操作",
        ...
        render: (handle, item) => {
          ...
          return (
            <div className="handle-box">
              <Icon
                type="eye"
                onClick={this.getDeviceDetail.bind(this, item)}
                title="查看详情"
                className={clName}
              />
              <Icon
                type="edit"
                onClick={() => this.showEditModal(item, "editOrder")}
              />
              <Icon type="delete" onClick={this.showConfirm.bind(this, item)} />
            </div>
          );
        }
      }
  ...
  render() {
    let { searchKey, data, editOrderName, handelType } = this.state;
    let { totalCount, content } = data;
    let count = totalCount % this.state.pageSize;
    let pageTotal =
      count > 0
        ? parseInt(totalCount / this.state.pageSize) + 1
        : totalCount / this.state.pageSize;
    ...
    return (
      <div className="order-container">
        ...
        <div className="order-head">
          ...
          <span>
            <Button
              type="primary"
              size="large"
              onClick={() => this.showEditModal(null, "addOrder")}
            >
              新增订单
            </Button>
          </span>
        ...
        <Modal
          title={handelType === "addOrder" ? "新增订单" : "修改订单名称"}
